document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelectorAll(".explore-btn-followw").forEach((button) => {
        button.addEventListener("click", function(){
            if (button.innerHTML == 'Follow'){
                suggested_followw(button)
            }else{suggested_unfollow(button)}
    ...
    document.querySelector("#js-input").onkeyup = function(event){
        if (event.which <= 90 && event.which >= 48 || event.which == 8) {
           if (event.which == 8 && document.querySelector("#js-input").value == '') {
            window.location.reload()
           } 
            console.log(event)
            //setTimeout(search_users, 5000)
            search_users()
            console.log("hi")
            //search_post(
        }
        ...
        console.log(event)
        setTimeout(function() {
            document.querySelectorAll("#explore-btn-follow-search").forEach((button) =>{
                console.log("nn")
                button.addEventListener("click", function() {
                    console.log("hid")
                    if (button.innerHTML == 'Follow') {
...
async function suggested_followw(button) {
    console.log("ran")
    const userf = button.parentNode.parentNode.querySelector("#explore-users-username").innerHTML
...
async function search_users() {
    ...
    console.log(data)
    document.querySelector('.users-search-profile-container').style.display = 'block';
    document.querySelector(".users-profile-container").style.display = 'none'; 
    const a = document.querySelector('.users-search-profile-container').querySelectorAll(".explore-users")
    const b = document.querySelector('.users-search-profile-container').querySelectorAll(".explore-users-extra")
    a.forEach((nod) => {
        console.log(nod)
        nod.remove()
    ...
    for (let i =0; i < data.length; i++) {
        try{
            if (data[i].username == (await curUser)) {continue;}
            const postContainer = document.querySelector(".explore-users").cloneNode(true)
            postContainer.querySelector("#explore-users-username").innerHTML = `${data[i].username}`
            postContainer.querySelector("#js-ahref").href = `/profile/${data[i].username}`
            postContainer.querySelector("#explore-users-profile-pic").src = `../medias/${data[i].profile_image}`
            postContainer.querySelector("#explore-users-bio").innerHTML = data[i].bio

            postContainer.querySelector(".explore-btn-followw").setAttribute("id", "explore-btn-follow-search")


            const response = fetch(`/is_following/${curUser}/${data[i].username}`)
            const followData = ((await response).json())
        
            if ((await followData) == true) {
                postContainer.querySelector(".explore-btn-followw").innerHTML = 'Following';
                postContainer.querySelector(".explore-btn-followw").style.backgroundColor = '#00CC66'
            }

            if ((await curUser) == postContainer.querySelector("explore-user-username")) {
                postContainer.querySelector(".explore-btn-followw").style.display = 'none';
            }

            document.querySelector('.users-search-profile-container').style.marginLeft = '130px';
            document.querySelector(".users-search-profile-container").append(postContainer)
            postContainer.style.borderColor='black';
    
        }
        catch(TypeError) {
            if (data[i].username == (await curUser)) {continue;}
            const postContainer = document.querySelector(".explore-users-extra").cloneNode(true)
...
async function explore_followw(button) {
    const userf = button.parentNode.querySelector("#explore-users-username").innerHTML
    const current_u = await getCurrentUser()
...
async function explore_unfollow(button) {
    const userf = button.parentNode.querySelector("#explore-users-username").innerHTML
    const current_u = await getCurrentUser()
